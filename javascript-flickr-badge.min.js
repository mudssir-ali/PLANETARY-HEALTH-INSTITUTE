...
function jsFlickrBadge(t, f) {
    ...
    var h = f.size,
        ...
        n = f.columns,
        r = f.animation,
        w = 1E3 * f.animationSpeed,
        u = 1E3 * f.animationPause,
        s = function(a, M) {
        ...
        x = function(a, b) {
            var f, h;
            for (f in a)
                if (a.hasOwnProperty(f) && (h = b(a[f], f), void 0 !== h)) break;
            return h
        },
        I = function(a) {
            var b = a ? a.split("px") : [];
    ...
    window[k] = function(a) {
        ...
        function k(a, l, d, c) {
            ...
            x(a, function(c) {
                f(c, {
                    top: p(b(c, "top")) + l.y + "px",
                    left: p(b(c, "left")) + l.x + "px"
                }, d)
            });
            window.setTimeout(c,
                d)
        ...
        function m(a, l, d) {
            ...
            null != l && null != d && (F.appendChild(c), e || (e = {
                left: p(b(c, "marginLeft")),
                right: p(b(c, "marginRight")),
                top: p(b(c, "marginTop")),
                bottom: p(b(c, "marginBottom")),
                boxShadow: b(c, E) || "",
                zIndex: p(b(c, "zIndex"))
            }, e.height = e.top + h + e.bottom, e.width = e.left + h + e.right, t.style.height = z * e.height + "px", t.style.width = n * e.width + "px"), a.coords = {
                row: l,
                col: d
            });
            c.href = a.link;
        ...
        function Q() {
            var a = s(n, A.x),
                ...
                m = {
                    height: 2 * h + e.top + e.bottom + "px",
                    width: 2 * h + e.left + e.right + "px"
                };
            k[E] = e.boxShadow;
        ...
        function R() {
            var b = [],
                a;
            for (a = 0; a < n; a++) b[a] = m(S(), z, a), v.push(b[a]);
            window.setTimeout(function() {
                k(v, {
                    x: 0,
                    y: -e.height
        ...
        function T() {
            ...
            0 < g.col && "r" != g.last && b.push({
                row: g.row,
                col: g.col - 1,
                last: "l"
            });
            g.col + 1 < q[0].length && "l" != g.last && b.push({
                row: g.row,
                col: g.col + 1,
                last: "r"
            });
        ...
        t.appendChild(F);
        for (a = 0; a < L; a++) y = S(), G = Math.floor(a / n), H = a % n, y = m(y, G, H), 0 == H ? q[G] = [y] : q[G][H] = y, v.push(y);
        C && ("shuffle" == r ? window.setTimeout(T, u) : "vscroll" == r ? (t.style.overflow = "hidden", R()) : "zoom" == r ? window.setTimeout(Q, u) : "flipX" == r ? K("X") : "flipY" == r && K("Y"))
    };
    b = "" + b + "&lang=en-us&format=json&jsoncallback=" +
        k;
